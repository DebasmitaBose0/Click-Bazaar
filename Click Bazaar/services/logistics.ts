...
export const generateOrderReceipt = (order: Order) => {
  ...
  doc.setFontSize(10);
  doc.text('OFFICIAL ORDER RECEIPT', 20, 33);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('LOGISTICS TIMELINE:', 120, 75);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.text(`Placed: ${addDays(createdAt, 0)}`, 120, 82);
  doc.text(`Shipment: ${addDays(createdAt, 1)}`, 120, 87);
  doc.text(`Delivery: ${addDays(createdAt, 3)}`, 120, 92);
  doc.text(`Arrival: ${addDays(createdAt, 5)}`, 120, 97);
  ...
  order.items.forEach((item) => {
    if (y > pageHeight - 40) {
      ...
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(10);
      doc.text(`CLICKBAZAAR - ${order.id} (Cont.)`, 20, 10);
      y = 25;
      drawTableHeader(y);
      y += 17;
    }
  ...
  if (y > pageHeight - 65) {
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(10);
    doc.text(`CLICKBAZAAR - Order Summary`, 20, 10);
    y = 30;
  }
  ...
  doc.text('Thank you for shopping at ClickBazaar India. This is a secure digital receipt.', 105, pageHeight - 15, { align: 'center' });

  doc.save(`ClickBazaar_Receipt_${order.id}.pdf`);
};